...
function plotData(data) {
    ...
    var width = 960,
        height = 480,
        xScale = d3.scaleLinear().domain([0, 360]).range([0, width]),
        yScale = d3.scaleLinear().domain([180, 0]).range([0, height]),
        rScale = d3.scaleLinear().domain([1, d3.max(masses)]).range([0.3, 20]),
        tooltip = d3.select('body').append('div').style('background', 'white').style('font-size', '10px').style('line-height', '100%').style('position', 'absolute').style('padding', '5px').style('border-radius', '3px').style('opacity', '0').style('box-shadow', '0 3px 5px 3px rgba(0,0,0,0.25), 0 3px 3px 2px rgba(0,0,0,0.22)');
    ...
    var chart = d3.select("#chart").select('svg').call(d3.zoom().on("zoom", zoomed2));

    var dots = d3.select('svg').append('g').attr('height', height).attr('width', width).attr('height', height).attr('width', width).selectAll('circle').data(data.features).enter().append('circle').attr('opacity', '0.7').attr('cx', function (d, i) {
    ...
    }).attr('fill', palette.red).on('mouseover', function (d, i) {
        d3.select(this).style('opacity', '0.8');

        tooltip.transition().style('opacity', '1');
        tooltip.html('Mass: ' + d.properties.mass + ' g<br>Name: ' + d.properties.name + '<br>Year: ' + d.properties.year.substring(0, 4) + '<br>Coordinates (Latitude : Longitude):<br> ' + d.geometry.coordinates[1] + ' : ' + d.geometry.coordinates[0]).style('left', d3.event.pageX + 8 + 'px').style('top', d3.event.pageY - 50 + 'px');
